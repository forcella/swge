...
public class DaoProduto extends DaoGenerica<Produto, Long> {
	...
	public List<Produto> listarTodosProdutosDiponiveis() {
		List<?> produtosDispo;
		Query query = entityManager.createQuery(
				"SELECT p FROM Produto p WHERE p.comercializavel is true");
		produtosDispo = (List<?>) query.getResultList();
		return (List<Produto>) produtosDispo;
	}
